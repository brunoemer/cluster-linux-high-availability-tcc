...
e \citet{zaminhani2008}. Os dois primeiros trabalhos implementam alta disponibilidade em máquinas virtuais, com mesmos objetivos deste trabalho. 
O terceiro autor utiliza uma técnica semelhante, porém implementada para apenas um serviço. Nestes trabalhos a estrutura é baseada em 
...
e um outro para monitoramento e gerência do \textit{cluster}. 

Para a escolha do \textit{software} de replicação de dados utilizou-se critérios que fossem compatíveis com as estruturas apresentadas nesses 
...
\begin{itemize}
 \item Integração com virtualização: possibilita a replicação dos dados das máquinas virtuais;
...
\begin{itemize}
 ...
 \item \textit{Failover} e \textit{failback} automáticos: move automaticamente os serviços do nó que falhou para um outro nó disponível.
...
\section{Definição do software para a replicação de dados}
...
dos dados de um servidor para um servidor em tempo real.
...
\begin{itemize}
 ...
 escrita de forma simultânea. Porém, este modo necessita de um sistema de arquivos compartilhados, sendo que neste caso pode ser utilizado os
...
\begin{itemize}
 ...
 \textit{bricks}. Esse tipo é normalmente utilizado para armazenamento de arquivos muito grandes e para balancear carga quando houver muito 
 acesso ao disco. Neste tipo de volume não tem-se nenhuma forma de replicação de dados;
...
\section{Definição do software para o gerenciamento de cluster}
...
que é responsável pela gerência e monitoramento do \textit{cluster}. MUDAR PROJETO IMPLEMENTACAO ??

No próximo capítulo será apresentado o projeto de implementação e serão descritos os testes criados e apresentado os resultados obtidos.
