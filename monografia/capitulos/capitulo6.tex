...
de virtualização e de \textit{softwares} de código aberto.
...
%virtuais entre os nós). 

%Usar metodologia MAH - Método Analítico Hierárquico \cite{geordano2014} NAO INICIALMENTE

Essa estrutura foi baseada em \citet{goncalves2009}, \citet{reis2009} e \citet{zaminhani2008}. Os dois primeiros trabalhos implementam alta 
disponibilidade em máquinas virtuais, com mesmos objetivos deste trabalho. O terceiro autor utiliza uma técnica semelhante, porém implementada 
para apenas um serviço. Nestes trabalhos a estrutura é baseada em \textit{cluster}\footnote[1]{Pode-se definir \textit{cluster} como um grupo 
de computadores interligados por rede com o objetivo de aumentar o desempenho ou disponibilidade de um serviço \cite{freitas2005}}, onde é 
utilizado um \textit{software} responsável pela replicação de dados e um outro para monitoramento e gerência do \textit{cluster} de alta 
disponibilidade. 

Desta forma, para a escolha dos \textit{softwares} procurou-se características que sejam compatíveis com as estruturas apresentadas nesses artigos
e que sejam adequadas ao ambiente da empresa. As características utilizadas para a escolha do \textit{software} de replicação de dados foram:
...
\begin{itemize}
 \item Integração com virtualização: possibilita a replicação dos discos das máquinas virtuais;
 \item Dual-primary: permite leitura e escrita de dados simultaneamente em ambos os nós;
 \item Replicação em tempo real: faz a replicação dos dados de forma instantânea e automática;
 \item Nível de replicação: pode ser a nível de bloco ou de arquivo;
...
As características utilizadas para a escolha do \textit{software} de monitoramento e gerência do \textit{cluster} foram:
\begin{itemize}
 \item Suporte nativo à virtualização: permite monitorar e manipular máquinas virtuais;
 \item Migração de \acp{VM} em tempo real: suporta a utilização de \textit{live migration} das máquinas virtuais, ou seja, possibilita mover uma
 \ac{VM} de um nó para outro sem reiniciá-lo;
 \item \textit{Failover} e \textit{failback} automáticos: move automaticamente os serviços de um nó que falhou para outro disponível;
...
dos dados de um servidor para um servidor remoto em tempo real.
Nas próximas seções tem-se uma descrição dos \textit{softwares} de replicação de dados que foram estudados, bom como as funcionalidades 
...
de \textit{cluster} e seu principal objetivo é a escalabilidade, ou seja, este possui funcionalidades que facilitam amplicar a capacidade 
...
\begin{itemize}
 ...
 \textit{bricks}. Esse tipo é normalmente utilizado para armazenamento de arquivos muito grandes e para balancear carga quando possuir muito 
 acesso a disco. Neste tipo de volume não tem-se nenhuma forma de replicação de dados;
...
\section{Softwares para o gerenciamento de cluster}
...
instância executará inicialmente; e o nó secundário, que será utilizado no caso de uma falha no nó primário.
...
Pode-se criar dependências e ordem entre esses recursos, para que esses sejam iniciados em uma determinada sequência. O \textit{Pacemaker} 
...
\subsection{Resumo e software de gerenciamento adotado}
...
Por fim, é o único que possui o sincronismo de configuração entre todos os nós. 
...
físicos, onde foram configuradas máquinas virtuais contendo os serviços críticos da empresa. Os \textit{softwares} utilizados foram escolhidos 
a partir de características compatíveis com alguns trabalhos científicos semelhantes e também adequadas com o ambiente da empresa. 
Deste modo, foram necessários dois \textit{softwares} para a implementação, o \ac{DRBD} para fazer a replicação dos dados, e o \textit{Pacemaker}
que é responsável pela gerência e monitoramento do \textit{cluster}. 

No próximo capítulo serão descritos os testes criados e apresentado os resultados obtidos.
