...
\begin{figure}[h!]
 ...
 \caption{Gráfico de requisições DNS (a). Comparação de requisições UDP entre os principais servidores (b).}
 \label{fig:dns_udp}
\end{figure}
...
os clientes não conseguirão estabelecer conexão para sua navegação. Os servidores \textit{Speedauth} e \textit{Masterauth} fornecem o serviço de
\textit{Radius}, sendo que recebem, em média, 1,6 requisições de autenticação por segundo, como pode ser observado na Figura 
\ref{fig:radius_auth_week} (a) e na \ref{fig:radius_auth_week} (b). Caso esses servidores ficarem indisponíveis aproximadamente 1,6 clientes 
...
de autenticação dos clientes. Como pode ser obervado na Figura \ref{fig:servico_elemento}, os servidores \textit{Speedauth} e \textit{Masterauth} 
...
pode ser observado na Figura \ref{fig:servico_tcp}.
...
entre os mais relevantes, como pode ser observado na Figura \ref{fig:servico_tcp}. 
