...
Com isso tem-se o ambiente pronto para a criação das \acp{VM}, sendo assim será criado uma instância para cada servidor virtual definindo
qual será o nó primário e o nó secundário. Como pode ser observado no Figura \ref{fig:projeto_estrutura}, será criado três instâncias no nó
\textit{master} e duas instâncias no outro nó, com isso possibilita-se o balanceamento de carga.
As instâncias criarão volumes lógicos \ac{LVM}\footnote{LVM é uma ferramenta de código aberto que possibilita a manipulação de discos rígidos, 
através da criação de grupos de volumes e volumes lógicos para \textit{Linux}.}  ?? VER ESSE PARAGRAFO
nos dois nós, para serem usados como disco das máquinas virtuais. Desta forma, quando houver 
uma falha no nó primário a instância será migrada para o nó secundário, sem que a máquina virtual seja reiniciada.
