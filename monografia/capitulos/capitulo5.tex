...
2 GB de memória \ac{RAM} e 24 GB de disco para cada sistema operacional hóspede. Além disso, optou-se por utilizar o mesmo sistema operacional e 
o mesmo hipervisor que são adotados atualmente na empresa, o sistema \textit{Ubuntu 14.04 \ac{LTS}} e o \ac{KVM} \cite{kvm}, respectivamente.
...
Para a replicação de dados será utilizado o \textit{software} \ac{DRBD}, sendo que para cada \ac{VM} existirá um disco \ac{DRBD} primário, que 
será replicado em um disco secundário localizado no outro nó do \textit{cluster}. O segundo \textit{software} será o \textit{Ganeti}, que fará o 
gerenciamento do \textit{cluster}. Esse será responsável pela gerência, monitoramento e migração das \acp{VM}. Para essas operações deve-se 
escolher um nó, que será o nó \textit{master}. Neste nó, serão realizadas as configurações iniciais do \textit{cluster}.
...
Deste modo, tem-se o ambiente pronto para a criação das \acp{VM}, sendo assim será criada uma instância para cada servidor virtual, e nela será
...
\textit{master} e duas instâncias no outro nó. De fato, no nó primário serão criadas as instâncias dos servidores \textit{Soldi}, \textit{Passata}
e \textit{Masterauth}. Já no nó secundário serão criadas as instâncias dos servidores \textit{SimplesIP} e \textit{Speedauth}.
...
e selecionada a mais adequada para o ambiente real da empresa. Além disso, foi apresentada a proposta de solução de alta disponibilidade.
No próximo capítulo são apresentados as conclusões parciais e o cronograma de andamento deste trabalho.
