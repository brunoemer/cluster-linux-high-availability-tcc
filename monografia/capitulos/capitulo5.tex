...
\textit{Pacemaker} \cite{pacemaker} é uma ferramenta de detecção e recuperação de falhas a nível de serviço \cite{perkov2011}. 
Essa ferramenta é um projeto de código aberto mantido pela \cite{clusterlabs}, sendo que ele é classificado como um \ac{CRM}, ou seja,
faz a gerencia de recursos de um \texit{cluster}, e teve origem com a necessidade de criar um gerenciador de recursos para a ferramenta 
\textit{Heartbeat}.

O \textit{Corosync} faz o sincronismo das configurações do cluster...

Continuar ??? \textit{Heartbeat} ou  \cite{zaminhani2008}.
