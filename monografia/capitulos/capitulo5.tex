...
Já o \textit{Ceph RBD} necessita de um \textit{hardware} adicional para sua administração, além de não ser uma escolha atrativa para um 
...
especificamente para virtualização e suporta os hipervisores \ac{KVM} e \textit{Xen}. Para sua implementação ele necessita de um nó 
\textit{master} e um nó \textit{master candidate} para armazenar as configurações do \textit{cluster}. Além disso, ele permite a criação de 
vários nós \textit{slaves}. As máquinas virtuais são chamadas de instâncias, sendo que em cada instâncias é configurado qual o nó primário 
que essa executará e qual o seu nó secundário.
...
\textit{failover} quando um nó falhar. Caso o nó \textit{master} falhar, suas funções serão transferidas para o \textit{master failover} ou 
\textit{master candidate}.
...
Pode-se observar que o \textit{software} \textit{Pacemaker} é o único que possui a característica de sincronismo de configuração entre todos 
os nós, no entanto isto não é um grande diferencial. Ele pode ser configurado para fazer monitoramento e recuperação tanto a nível de nó quanto 
a nível de recurso, desta forma dificultando a sua configuração. Devido ao monitoramento a nível de recurso, a ferramenta torna-se flexível para 
execução com diversas aplicações, porém não obteve-se o resultado esperado nos testes efetuados em um ambiente de virtualização.
...
O \textit{software} escolhido para gerenciar o ambiente que será criado neste trabalho foi a ferramenta de código aberto \textit{Ganeti}. 
Esse \textit{software} possui todos os requisitos para criar um \textit{cluster} de alta disponibilidade, além de possuir a vantagem de ser 
desenvolvido especificamente para virtualização, o que simplifica a sua configuração. O \textit{Ganeti} possui detecção de falhas tanto a nível
de nó quanto a nível de máquina virtual. Nos testes realizados destaca-se a opção a migração em tempo real, pois obteve-se um bom resultado.
...
\textit{cluster}. O primeiro será o \ac{DRBD} e o segundo será o \textit{Ganeti}, ambos podem ser observados na Figura 
...
replicação \ac{DRBD}. E para utilizar esse dispositivo como disco de uma máquina virtual será criado um volume lógico através da ferramenta 
...
%Para configurar o ambiente será necessário instalar os \textit{softwares} do \textit{cluster}...
...
hipervisor \ac{KVM}. A Figura \ref{fig:vms_migration} demonstra dois servidores com um hipervisor e duas \ac{VM}s executando. Inicialmente 
estas estão executando sobre o primeiro servidor, então a \ac{VM} 1 é migrada para o segundo servidor \cite{livemigration}.
