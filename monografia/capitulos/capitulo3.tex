...
pela \textit{IBM} e foi um dos principais sistemas comerciais com suporte à virtualização da época. Esse sistema operacional executava 
...
um \ac{PC} para cada usuário, ao invés de investir em \textit{mainframes}. Devido ao crescente avanço e o melhor desempenho do \ac{PC} e
...
\begin{itemize}
 ...
 \begin{itemize}
  ...
  executam em modo não-privilegiado. Um exemplo de instrução de usuário é uma instrução do processador que executa diretamente sobre a memória
...
aplicação e suportam apenas um processo ou aplicação. Um exemplo de máquina virtual de aplicação é a \ac{JVM}. Enquanto uma máquina virtual de 
...
\begin{itemize}
 ...
 ser executado em uma máquina virtual. Sendo assim para cada arquitetura ou sistema operacional deve existir uma máquina virtual que
...
que um ou mais sistemas operacionais convidados executem sobre o mesmo computador físico independentemente. O hipervisor provê uma interface
...
Virtualização de sistema utiliza abstração em sua arquitetura, por exemplo, ela transforma um disco rígido físico em dois discos rígidos
...
Existem basicamente duas arquiteturas de hipervisor de sistema, que são apresentadas na Figura \ref{fig:vms_arquiteturas} e detalhadas
...
Sabendo essas definições das arquiteturas pode-se concluir que hipervisores convidados são mais flexíveis que os nativos, pois podem ser 
implementados em diversos sistemas operacionais e \textit{hardwares}. Já os hipervisores nativos possuem melhor desempenho pois acessam o 
...
\begin{itemize}
 ...
 todas as chamadas de sistemas e operações do sistema convidado. Um exemplo de virtualização total é o \textit{QEmu};
