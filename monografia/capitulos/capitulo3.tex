...
tipos de interfaces existentes em sistemas de computação (Figura \ref{fig:interfaces_isa}) \cite{maziero2013}:
\begin{itemize}
 ...
 \begin{itemize}
  \item Instruções de usuário ou \textit{User \ac{ISA}}: são instruções disponíveis à aplicações de usuários. Instruções de usuário são instruções 
  ...
  Caso uma instrução privilegiada for executada no modo usuário, ela manifesta-se através de uma interrupção \cite{buyya2013};
  ...
  \item Instruções de sistema ou \textit{System \ac{ISA}}: são instruções exclusivamente acessíveis ao núcleo do sistema operacional, que 
  são instruções privilegiadas, ou seja, são instruções que acessam recursus compartilhados. Essas instruções são executadas em modo supervisor 
...
%Por Rajkumar Buyya,Christian Vecchiola,S.Thamarai Selvi
