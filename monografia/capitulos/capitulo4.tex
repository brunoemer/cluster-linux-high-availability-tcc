...
A maioria dos serviços são fornecidos por meio de \textit{softwares} de código aberto, onde executam em sistemas operacionais, normalmente 
distribuições \textit{Linux}, que também são \textit{softwares} livre.
...
equipamentos como por exemplo roteadores, de forma que caso um falhe o outro alimente todos os equipamentos. O terceiro \textit{nobreak} é 
...
o modelo do servidor, a configuração dos processadores, quantidade de memória, número de discos e a capacidade unitária de cada disco.
...
pode-se dobrar a capacidade de tráfego de dados e assim é caracterizado a redundância do cabeamento.
...
Existem sete servidores que possuem serviços que executam sobre o sistema operacional nativo, ou seja, sem virtualização. Eles são os sete 
...
\begin{itemize}
 ...
 \textit{Cacti 0.8.8a}. Esse servidor monitora atualmente o tráfego dos clientes, tanto de internet via rádio, como de fibra óptica;
 
 \item Vigilante: esse servidor é responsável por capturar e armazenar \textit{streaming} de vídeo de câmeras do provedor. Ele possui
...
geralmente utilizando \ac{RAID} 5, em alguns casos, quando possuem apenas dois discos é utilizado \ac{RAID} 1 (espelhamento de discos). O 
ambiente também possui uma redundância do cabeamento, como visto anteriormente.
...
são utilizados para virtualizar sistemas, que são os últimos sete servidores da Tabela \ref{tab:servfisicos}.
...
\begin{itemize}
 ...
 operacional \textit{Ubuntu 14.04 \ac{LTS}}. Esse servidor provê um serviço de mensagens instantâneas, baseado no protocolo \ac{XMPP}, o que 
 ...
 sistema operacional \textit{Ubuntu 14.04 \ac{LTS}}. Ele fornece um serviço de teste de velocidade da conexão de internet, os 
