...
\textit{e-mail marketing}, \textit{backup}, máquinas virtuais, autenticação via \ac{ADSL}, rádio \textit{online} e telefonia.
...
A redundância de energia é feita através de três \textit{nobreaks}, sendo que dois deles (identificados como NB 1 e NB 2 na Figura 
...
Atualmente existem sete servidores que possuem serviços que são executados sobre o sistema operacional nativo, ou seja, sem virtualização. 
...
\begin{itemize}
 ...
 suportem o protocolo \ac{SNMP} \cite{??}. Essa ferramenta monitora a maior parte da rede \textit{core} e da rede \textit{backbone}, tanto dos clientes de 
 ...
 do disco rígido, da temperatura e da velocidade dos \textit{fans}\footnote[1]{Os \textit{fans} ... ?? };
...
um \ac{RAID} \cite{??}. Em servidores com mais de dois discos é utilizado \ac{RAID} 5 \cite{??}. Já em servidores que possuem apenas dois 
discos é utilizado \ac{RAID} 1 (espelhamento de discos) \cite{??}. O ambiente também possui uma redundância do cabeamento de rede, 
...
\begin{itemize}
 \item \textit{Masterauth}: sua configuração é 1 \textit{core} de \textit{2.33 GHz}, 1,5 GB de memória RAM e 8 GB de disco. 
 ...
 \ac{PPPoE} \cite{??} para uma parte dos clientes do provedor. Para essa autenticação é utilizada o \textit{software} 
 \textit{Freeradius 2.1.12} \cite{freeradius};
 
 \item \textit{Monete}: sua configuração é 1 \textit{core} de \textit{2.33 GHz}, 3 GB de memória RAM e 50 GB de disco. 
...
\begin{itemize}
 \item \textit{Hotspot}: sua configuração é 1 \textit{core} de \textit{2.0 GHz}, 1,5 GB de memória RAM e 8 GB de disco. 
 ...
 da \textit{Ubiquiti} que fazem \textit{hotspot}\footnote[1]{Hotspot é ...}. Ou seja, esses equipamentos disponibilizam a tecnologia 
 ...
 \item \textit{Ottico}: esse servidor possui 2 \textit{cores} de \textit{2.0 GHz}, 4 GB de memória RAM e 50 GB de disco. 
 ...
 \item \ac{PRTG}: esse servidor possui 2 \textit{cores} de \textit{2.0 GHz}, 4 GB de memória RAM e 100 GB de disco. 
 ...
 \item \textit{Passata}: esse servidor possui 2 \textit{cores} de \textit{2.0 GHz}, 3 GB de memória RAM e 20 GB de disco. 
 ...
 \item \textit{Roncon}: esse servidor possui 4 \textit{cores} de \textit{2.0 GHz}, 6 GB de memória RAM e 400 GB de disco. 
 ...
 Além da hospedagem, esse servidor fornece o serviço de autenticação \ac{ADSL} \cite{??} de terceiros utilizando o \textit{software} 
 \textit{Freeradius 1.1.3} \cite{freeradius};
 
 \item \textit{Servo}: sua configuração é 1 \textit{core} de \textit{2.0 GHz}, 2 GB de memória RAM e 30 GB de disco. 
 ...
 \item \textit{SimplesIP}: esse servidor possui 2 \textit{cores} de \textit{2.0 GHz}, 3 GB de memória RAM e 80 GB de disco. 
...
\begin{itemize}
 \item \textit{ASP}: esse servidor possui 1 \textit{core} de \textit{2.40 GHz}, 1 GB de memória RAM e 50 GB de disco. 
 ...
 \item \textit{CactiBackbone}: esse servidor possui 1 \textit{core} de \textit{2.40 GHz}, 1 GB de memória RAM e 20 GB de disco. 
 ...
 \item \textit{Dio}: esse servidor possui 1 \textit{core} de \textit{2.40 GHz}, 1 GB de memória RAM e 17,8 GB de disco. 
 ...
 \item \textit{FateFurbo}: sua configuração é 2 \textit{cores} de \textit{2.40 GHz}, 4 GB de memória RAM e 80 GB de disco. 
 ...
 \item \textit{FiberHome}: sua configuração é 2 \textit{cores} de \textit{2.40 GHz}, 2 GB de memória RAM e 60 GB de disco. 
 ...
 \item \textit{Parla}: sua configuração é 1 \textit{core} de \textit{2.40 GHz}, 1 GB de memória RAM e 8 GB de disco. 
 ...
 \ac{XMPP} \cite{??}. Esse serviço é utilizado para comunicação entre funcionários da empresa e do provedor, e também entre clientes e 
 ...
 \item \textit{Passata2}: esse servidor possui 1 \textit{core} de \textit{2.40 GHz}, 2 GB de memória RAM e 20 GB de disco. 
 ...
 \item \textit{Postfix}: sua configuração é 1 \textit{core} de \textit{2.40 GHz}, 768 MB de memória RAM e 50 GB de disco. 
 ...
 \item \textit{Servo6}: sua configuração é 1 \textit{core} de \textit{2.40 GHz}, 1,5 GB de memória RAM e 30 GB de disco. 
...
através de um serviço de acesso remoto, como por exemplo, através de \ac{SSH} \cite{??}.
...
\begin{itemize}
 \item \textit{Ledriovardar}: esse servidor possui 2 \textit{cores} de \textit{2.40 GHz}, 2 GB de memória RAM e 80 GB de disco. 
 ...
 10 GB de memória RAM e 1 TB de disco. O servidor possui o sistema operacional \textit{Windows 2008 Server R2} e executa o \textit{software} 
 ...
 \ac{SMTP} \cite{??}, recebimentos de \textit{e-mails} através dos protocolos \ac{POP} \cite{??} e \ac{IMAP} \cite{??}, 
 ...
 \item \textit{Quebei}: sua configuração é 1 \textit{core} de \textit{2.00 GHz}, 3 GB de memória RAM e 140 GB de disco. 
...
\begin{itemize}
 \item \textit{Mondoperso}: sua configuração é 1 \textit{core} de \textit{2.53 GHz}, 512 MB de memória RAM e 8 GB de disco. 
 ...
 \item \textit{Ns}: esse servidor possui 1 \textit{core} de \textit{2.53 GHz}, 2 GB de memória RAM e 30 GB de disco. O servidor possui o sistema 
 ...
 \item \textit{Soldi}: sua configuração é 4 \textit{cores} de \textit{2.53 GHz}, 4 GB de memória RAM e 40 GB de disco. 
 ...
 \item \textit{Speedauth}: sua configuração é 2 \textit{cores} de \textit{2.53 GHz}, 1,5 GB de memória RAM e 8 GB de disco. 
...
\begin{itemize}
 \item \textit{Backup}: sua configuração é 1 \textit{core} de \textit{3.10 GHz}, 1 GB de memória RAM e 15 GB de disco. 
 ...
 \item \textit{Esibire}: sua configuração é 1 \textit{core} de \textit{3.10 GHz}, 1 GB de memória RAM e 50 GB de disco. 
 ...
 \item \textit{Miatanto}: sua configuração é 1 \textit{core} de \textit{3.10 GHz}, 1 GB de memória RAM e 8 GB de disco. 
 ...
 \item \textit{Pomodoro}: sua configuração é 1 \textit{core} de \textit{3.10 GHz}, 2 GB de memória RAM e 28 GB de disco. 
 ...
 \item \textit{Trapel}: sua configuração é 1 \textit{core} de \textit{3.10 GHz}, 768 MB de memória RAM e 8 GB de disco. 
