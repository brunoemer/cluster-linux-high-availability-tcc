...
Para selecionar os serviços mais utilizados pode-se compará-los utilizando um valor em comum entre todos os servidores, que é o número de 
conexões \ac{TCP} e transmissões \ac{UDP}. Além disso, será utilizado o número de elementos de cada serviço, como por exemplo a quantidade de 
contas de \textit{e-mail} em um servidor de \textit{e-mail} e a quantidade de equipamentos monitorados por um servidor de monitoramento. 
Abaixo está a Tabela X??



Os seguintes critérios serão utilizados para definir esses serviços críticos: 
\begin{itemize}
...
\end{figure}
 ...
 outros. Esse serviço não tem um impacto direto para os clientes, porém tem um grande impacto para os funcionários da empresa e do provedor. 
 ...
 empresa e o provedor. Outro critério que pode ser utilizando para esse serviço são as requisições por segundo, que são recebidas pelo sistema 
 principal do provedor e ficam aproximadamente entre 3 e 4 por segundo, conforme Figura \ref{fig:soldi_week};
...
\end{figure}
 ...
 No mês de maio de 2016 o serviço recebeu xx ligações no total (Tabela xx??), sendo que essas ligações são utilizadas por todos o setores do 
 provedor e também da empresa. O gráfico da Figura \ref{fig:simplesip_week} mostra a quantidade de canais ativos simultaneamente no servidor 
 de telefonia, com isso pode-se perceber que existem de 20 e 30 ligações que são feitas durante o horário comercial.
\end{itemize}
