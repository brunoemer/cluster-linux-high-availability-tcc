...
as falhas ocorram. Além disso, a prevenção de falhas melhora a disponibilidade e a confiabilidade de um serviço, uma vez que esta 
...
Pode-se utilizar virtualização para tornar um sistema tolerante a falhas. Nestes ambientes normalmente existem dois servidores 
...
Neste capítulo foram descritos os principais conceitos de alta disponibilidade e conceitos relacionados. Pode-se concluir que a alta 
disponibilidade é alcançada através da tolerância a falhas, que por sua vez utiliza redundância para mascarar falhas. Sendo que a redundância 
reduz os \ac{SPOF}. 

Como mencionado anteriormente, um dos principais recursos utilizados atualmente para a obtenção de alta 
disponibilidade é a virtualização, uma vez que essas são utilizadas para implementar a redundância de \textit{software}. Desta forma, no 
próximo capítulo será feita uma breve definição de virtualização, com as vantagens, as estratégias de implementação e os grupos de 
máquinas virtuais.
